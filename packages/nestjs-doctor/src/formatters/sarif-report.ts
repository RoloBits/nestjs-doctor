...
/**
 * Builds the rule catalogue.
 *
 * Custom rules and any rule the config disabled are absent from `getRules()`
 * output for a given run, so a diagnostic can reference a rule the catalogue
 * doesn't know. Those get a synthesised entry from the diagnostic itself —
 * `ruleIndex` must resolve or GitHub rejects the whole upload.
 */
...
/**
 * Renders a result as a SARIF 2.1.0 log for GitHub code scanning.
 *
 * `partialFingerprints` is populated explicitly: without it GitHub derives its
 * own from surrounding source, and an unrelated edit above a finding then
 * closes the old alert and opens an identical new one.
 */
