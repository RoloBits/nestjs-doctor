...
/**
 * Works out which files (and lines) the reported set should be narrowed to.
 *
 * Every failure path degrades to a wider scope with a warning rather than
 * erroring: a scan that reports too much is recoverable, one that reports
 * nothing because git was unavailable silently hides real findings.
 */
...
/**
 * Narrows a diagnostic set to the resolved scope.
 *
 * `changed` is not handled here — a baseline delta needs a second scan and is
