...
/**
 * GitHub renders at most 10 error and 10 warning annotations per step (50 per
 * job), and silently drops the rest. Emitting more just wastes log lines — the
 * job summary and the pull request comment carry the complete set.
 */
...
/**
 * Builds the workflow-command annotation lines for a result, capped per level.
 *
 * Errors come first within each level so the ten that survive the cap are the
 * ones most worth surfacing on the diff.
 */
 ...
 *
 * Both channels are best-effort: a missing or unwritable `GITHUB_STEP_SUMMARY`
 * must not fail a scan that otherwise succeeded.
 */
