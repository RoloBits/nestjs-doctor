...
- 0c09973: The HTML report reports how it is used, alongside the tab it was opened on: a
...
- 0c09973: A scan now reports which well-known packages a project depends on: the NestJS
...
- ee09c74: A scan now reports anonymous counts of which built-in rules fired, were
...
- 0c09973: Reporting uses a stable anonymous install id kept in a config file, instead of a
...
- 0c09973: A scan now reports how a project is configured: the score threshold, which
  ...
  include, exclude and ignored-file patterns the project declared.
...
- 7bb9093: `--output` now names where `--report` writes the HTML.
...
- 8006d2e: The HTML report now reports which tab was opened. It sends two events,
...
- 6261fa9: Report how a CI scan was triggered, so the GitHub Action's own usage is
  measurable.
...
- 7bb9093: Say when no dependency could be checked, instead of reporting nothing.
...
- db966da: Trim the report telemetry notes to one sentence on the report docs page. The
  README and the CLI flag tables no longer carry them.
- ba77aa5: Reviewing a finding in the menu now shows the rule's own guidance underneath it: what the rule looks for, then the bad and good code samples the HTML report already carried. Bad and good are marked by a word, a glyph and a per-line sigil, so the pair reads the same with colour turned off.
...
- 0c09973: A config file that will not parse now fails the scan instead of being ignored.
...
- e31153d: The post-scan menu gains a finding browser: rule groups ordered worst first, a per-finding view with the code frame, the fix guidance and the docs link, and a copyable agent-ready fix prompt per rule.
- 339f80b: The post-scan menu can hand the findings to a coding agent: Claude Code, Codex, or Cursor detected on PATH start with the top rule groups as their first prompt, without permission-skipping flags, or the prompt is copied for any other agent.
...
- a1c6317: The scan now shows live progress: the spinner animates instead of freezing on one frame, a progress bar fills through the parse, analysis and rule phases, and monorepo scans label each sub-project. The rule pass yields to the event loop between file batches, so terminal output streams during the scan instead of appearing all at once at the end. Interactive runs now stop at the score box instead of dumping every finding; the menu owns the findings, and `--verbose` still prints them.
- 38eb780: Turn on the anonymous rule-count reporting and the HTML report's beacon, both of
...
- 7bb9093: Make a monorepo's sub-project breakdown add up to its combined totals.
...
- 7bb9093: Report the installed NestJS version, not the declared range.
...
- 1ab1256: Restyle the HTML report to match the docs site and landing page: IBM Plex Mono throughout (UI, code editors, and the graph canvases), a black full-width nav with an uppercase wordmark and a bordered GitHub button, uppercase tab labels, and square-cornered summary cards with a bordered caption strip.
...
- 540fd52: Link the module graph docs from the readme's report section.
...
- d3c440a: Point the Rule Lab's custom-rules link at the rule-authoring docs page, which moved to /docs/rules/custom.
- 1ab1256: Lay out the schema diagram in layered columns (referenced hubs left, referencing tables right, barycenter-ordered rows) and route every relation orthogonally through the gutters between columns, with per-gutter lane spreading, port fan-out on hub tables, and box-avoiding crossings. Replaces the dagre-based placement and the diagonal fallback routes that made large schemas unreadable.
