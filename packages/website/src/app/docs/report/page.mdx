...
The button in the route's header row copies the walk as plain text: the route
and its handler, the verdict with what it means, the steps the default filter
keeps with their flags and call sites, every condition or guard above them, and
the source lines of the first read and the first write. Paste it into whichever
agent you use. The same text sits on each endpoint as `explain` in
`--format json`, byte for byte.
...
how to read the result, and the button that copies a trace for an AI agent.
