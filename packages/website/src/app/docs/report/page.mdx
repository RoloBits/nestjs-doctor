...
The fifth is anonymous usage reporting, and unlike the others it sends more
than once: the tab you opened, a fixed name for each interaction it counts
(running the rule lab, expanding a tree, opening a module from a finding), and
your click positions as percentages of the window, so the layout can be
improved. Each carries the nestjs-doctor version and whether the file came from
CI or the CLI. Every name it sends is one of a fixed list compiled into the
report, so no file path, module name, or source text from your project travels
with it. `telemetry: false` in your config turns it off before the report is
written.
