...
| Element | Where | Means |
|---|---|---|
| Overview lane | Top of the lanes | `building modules · lifecycle hooks · opening the port`, from the `createMs`, `initMs`, and `startupMs` markers; with `moduleInitMs` the hooks split into `init hooks · bootstrap hooks`. The window on top is the view: drag it to pan, drag its edges to resize, scroll to zoom, click a phase to zoom to it |
| Dotted line | Down the rows | Where one phase hands over to the next |
| Class bar | Per row, grouped by module | From its slowest dependency's finish to its own, or from boot start when nothing traced came before; coloured by type. The time it took sits inside the bar |
