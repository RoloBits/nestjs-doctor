...
The button above the timeline copies the active trace as plain text: how long
the boot took and its phases, the last class built and what it waited on, the
ten slowest modules and classes with their own time after their dependencies,
and the slowest hooks. Paste it into whichever agent you use. A `--json` run
with `--timings` carries the same text under `boot`, one entry per trace.
