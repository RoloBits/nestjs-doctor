...
Wrap the bootstrap in `src/main.ts`, for development only:
...
wraps each instance's hooks so their durations land in `hookTimings`.
...
Below Nest 11.1.4, drop `instrument` and `hookTimings`. Class construction
times still work.
