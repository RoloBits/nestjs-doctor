...
- **Schema-scoped** (`scope: "schema"`) — runs once per schema graph. Gets a `SchemaRuleContext` with the extracted entity-relation data from Prisma, TypeORM, Drizzle, or MikroORM.
...
| Property | Type | Description |
|----------|------|-------------|
| `schemaGraph` | `SchemaGraph` | Entity-relation data extracted from Prisma, TypeORM, Drizzle, or MikroORM |
| `orm` | `string` | Detected ORM name (e.g. `"prisma"`, `"typeorm"`, `"drizzle"`, `"mikro-orm"`) |
| `report()` | `(diagnostic) => void` | Emit a diagnostic |
