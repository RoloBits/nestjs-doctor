...
| `no-advisory-nestjs-packages` | warning | An `@nestjs/*` version with a moderate or low advisory |
...
Reports an `@nestjs/*` package on a version with a critical or high advisory.
...
- otherwise nothing, for a spec like `workspace:*` or `11.x` that names no version
...
`package.json` takes no comments, so neither inline directive reaches these findings. Silence one with `ignore.rules` in your config file.

Under `--scope changed` or `--scope files` the finding appears only when `package.json` itself changed, which is the scope the GitHub Action uses by default.
