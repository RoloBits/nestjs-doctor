...
**Why:** Direct `process.env` access scatters configuration across the codebase, makes testing harder, and bypasses NestJS's `ConfigService` validation.
...
**Why:** ORM entities may contain internal fields (passwords, soft-delete flags, relation metadata) that shouldn't be exposed. Use DTOs to control the response shape.
