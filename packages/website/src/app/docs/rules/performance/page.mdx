...
A provider a module exports is skipped, whether the export is in the `@Module()` decorator or in the `DynamicModule` a `forRoot()` or `register()` returns.
...
A provider named in any module's `exports` array is skipped as well, because a consumer outside the scan may inject it.
...
To find a consumer, the rule reads the constructor parameters and `@Inject()` tokens of every provider and controller in the importing modules, including providers the importing module registers through a `DynamicModule`.
