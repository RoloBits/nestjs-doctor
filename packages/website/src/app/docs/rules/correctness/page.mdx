...
A `providers` array inside a `DynamicModule` counts: the object returned by `forRoot()`, `register()`, a static method of any name, a standalone function, or a `ConfigurableModuleBuilder` `setExtras` callback. The array can be a local variable, a spread, a `push`, a helper call, or a ternary.
