...
The rule traces provider-level dependencies to find what creates each module edge. It then names the providers to extract into a shared module, choosing the edge with the fewest of them.

Treat `forwardRef()` as a last resort. It defers resolution at runtime and leaves the architectural coupling in place.

A reported cycle lists each edge and the suggested extraction:
