...
**Why:** Controllers should be thin — they receive a request, delegate to a service, and return the response. Business logic in controllers can't be reused, is harder to test, and violates the single responsibility principle.
...
**Why:** Same principle as `no-repository-in-controllers` but for other ORM access patterns. Controllers should go through a service layer.
...
**Why:** Circular module dependencies make the dependency graph unpredictable, cause initialization ordering issues, and make the codebase harder to reason about.
...
**Why:** Manually instantiating injectable classes bypasses the DI container, so constructor dependencies aren't resolved. It also breaks scoping and lifecycle hooks.
...
**Why:** The service locator pattern hides dependencies — you can't see what a class depends on by looking at its constructor. Use constructor injection instead.
...
**Why:** Constructor injection makes dependencies explicit and immutable. Property injection hides dependencies and allows them to be `undefined` before initialization.
...
**Why:** Importing from `../other-module/services/internal.service` creates tight coupling to the other module's structure. Import from the module's public API (barrel file) instead.
...
**Why:** Barrel files define a module's public API. Re-exporting repositories or internal services encourages consumers to bypass the service layer.
