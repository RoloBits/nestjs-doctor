...
| `no-orm-in-controllers` | error | ORM types in controllers: PrismaService, EntityManager, EntityRepository, DataSource, DrizzleService, MikroORM, MongooseModel |
...
Detects ORM types (`PrismaService`, `PrismaClient`, `EntityManager`, `EntityRepository`, `DataSource`, `Repository`, `Connection`, `MongooseModel`, `MikroORM`, `DrizzleService`) and ORM-binding decorators (`@InjectRepository`, `@InjectModel`, `@InjectEntityManager`) injected directly in controllers.
...
The default is unchanged: cycles are reported as `error` whatever `forwardRef()`
does.
...
Detects services using ORM types directly (`PrismaService`, `EntityManager`, `DataSource`, `Connection`, `MongooseModel`, `MikroORM`, `DrizzleService`) or ORM-binding decorators (`@InjectRepository`, `@InjectModel`, `@InjectEntityManager`) instead of going through a repository layer.
...
> **Note:** Typed repository wrappers — TypeORM's `Repository<T>` and MikroORM's `EntityRepository<T>` — are intentionally **allowed** in services because they are themselves the canonical repository abstraction. Classes named `*Repository`/`*Repo` are also skipped (so a custom repository implementation can inject whatever it needs).

> **Note:** If your project follows the [official NestJS Prisma recipe](https://docs.nestjs.com/recipes/prisma) (injecting `PrismaService` directly) or the [official @mikro-orm/nestjs setup](https://mikro-orm.io/docs/usage-with-nestjs) (injecting `EntityManager` / `@InjectRepository`), you can disable this rule in your configuration.
