...
turning a pull request red. Enforcement is off until you set `blocking` or `min-score`.

Two independent gates decide the outcome. Either one failing fails the run.
...
Because it follows the scope, a repository
...
The score **always reflects the whole project**, whatever `scope` says.
Narrowing a report must never make a codebase look healthier than it is.

So `min-score` gates the repository's health, not the change. A pull request
touching one file can fail it because of debt somewhere else.
...
Use `blocking` alone for new code. Add `min-score` for a floor on the
...
[Pushes to the default branch](/docs/ci#pushes-to-the-default-branch).

## Gates on the command line

The same two gates the action exposes, as flags. Everything above applies here too — what each gate sees,
