...
The comment says which happened. `silence-missing-baseline-warning` hides the
notice if you would rather live with the fallback.
...
with the same rules and config, and subtracts what was already there.

- Findings match on rule, file, message, and source text. Never on line number,
  so an edit higher up a file does not make an old finding look new.
- The comment also reports how many findings the change **resolved**.
