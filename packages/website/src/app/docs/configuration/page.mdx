...
The rule list is space- or comma-separated; omit it to suppress every rule for that scope. A `-- reason` trailer is ignored, so you can document the exception inline. See [Diagnostic Filtering](/docs/pipeline/diagnostic-filtering) for details.
