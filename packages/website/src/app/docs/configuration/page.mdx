...
Set a rule to `false` to disable it, or to an object to override its severity:
...
|---|---|---|
| `nest-cli.json` | `"monorepo": true` plus a non-empty `projects` map | Each entry's `root` is the sub-project path |
...
| `nx.json` | every `project.json` beside it | A project with no `package.json` counts when one of its `*.module.ts` files imports `@nestjs/common` |
...
Every other strategy resolves its globs to packages, then keeps only those depending on `@nestjs/core` or `@nestjs/common`. If none qualify, nestjs-doctor warns and falls back to single-project mode.
