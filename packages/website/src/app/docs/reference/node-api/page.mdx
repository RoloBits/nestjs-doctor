...
graph.entries; // EntryPoint[], one per traced HTTP route
```
...
path of the declaring file, the same one the endpoint graph reports. Four
variations sit inside that shape:
...
for (const id of reachableFrom(graph, [entry.node])) {
