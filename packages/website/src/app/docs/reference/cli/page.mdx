...
`changed` needs history. In CI, check out with `fetch-depth: 0`, or it falls
back to reporting every finding in the changed files and says so.
...
`--blocking` defaults to `error` for the console report and `none` for `--json`
and `--score`, because those exit codes predate the flag. Set it explicitly and
every mode agrees.
