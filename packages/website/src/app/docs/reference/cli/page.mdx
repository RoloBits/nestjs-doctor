...
`files`, `lines` and `changed` keep a finding whose file sits above the scanned
directory. The changed-file set describes the scanned tree only, so a
`package.json` one level up is never known to be unchanged.
