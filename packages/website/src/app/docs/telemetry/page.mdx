...
A finished `--init` or `ci install` reports one `command_completed` event with two fields: `command`, either `init` or `ci_install`, and `from`, either `flag` or `menu`. It carries the install id and nothing about the project.
...
| `trigger` | How the run started: `action`, `ci`, `hook`, `skill`, `agent`, `script`, `npx` or `global` |
...
`NESTJS_DOCTOR_TRIGGER` overrides the detected `trigger` when set to one of the eight values above. Any other value is ignored. `skill` is never detected: the shipped skills set it themselves, so a run an agent starts through the skill lands in `skill` and one it starts on its own lands in `agent`.
