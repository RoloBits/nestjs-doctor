...
The NestJS version is read from `node_modules/@nestjs/core`, walking up from the
scanned path so a hoisted workspace install is found. The walk stops at the
first directory holding a `.git`.

With nothing installed it falls back to the first version the declared spec
names, so `^10.0.0` reads as `10.0.0`. A spec naming no version, such as
`workspace:*`, leaves `nestVersion` as `null`.
