...
Users need to customize which files to scan, which rules to enable/disable, and which diagnostics to ignore. The config loader provides a fallback chain so nestjs-doctor works out of the box but is fully customizable.
...
- If a file isn't being scanned, check if it matches an exclude pattern. The default excludes are extensive (test files, mocks, seeders, declaration files).
- If a rule isn't running, check both `rules` (individual toggle) and `categories` (category-level toggle) in config.
