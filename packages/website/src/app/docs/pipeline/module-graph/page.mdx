...
- **Unresolvable expressions:** ternaries and computed values return empty. The import is skipped, with no crash.
...
- If a cross-file function or variable is not resolved, verify that it is exported from its source file. It must also be imported with a named import in the module file. Only relative paths (`./` or `../`) are followed, so bare specifiers like `@nestjs/common` are skipped.
