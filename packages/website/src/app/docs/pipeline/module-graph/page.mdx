...
Names imported from a bare package specifier, meaning anything that is not a
relative path or a tsconfig path alias, are recorded in `packageImports` with
their specifier.

In a monorepo merge, such a name binds to a module in another sub-project only
when the specifier, or a parent of it, is itself a scanned workspace package
that declares the module.
