...
The rule list is space- or comma-separated; omitting it suppresses every rule for that scope. A `-- reason` trailer is ignored. The source text is resolved from the AST project (no extra disk reads) and each file is parsed at most once. Because line-scoped directives key off the diagnostic's `line`, they apply only to code diagnostics — schema diagnostics (which have no line) are suppressed only by `nestjs-doctor-ignore-file`.
