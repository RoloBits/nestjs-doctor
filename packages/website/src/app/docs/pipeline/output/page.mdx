...
Each endpoint also carries `explain`, the route's walk as plain text: the
database verdict, the steps with their call sites, the conditions above them,
and the source lines of the first read and write. It is the text the report's
**Copy for AI agent** button copies, and it needs the code graph, so a `--json`
run builds one:

```bash
npx nestjs-doctor . --json | jq -r '.endpoints.endpoints[] | select(.routePath == "/orders/:id") | .explain'
```

With `--timings`, the result also carries `boot`, one entry per trace with its
`label`, `project` and the boot as plain text under `explain`:

```bash
npx nestjs-doctor . --json --timings nestjs-doctor-timings.json | jq -r '.boot[].explain'
```
