...
- **Endpoints:** traced HTTP routes per controller with their dependency chains. Controllers and handlers behind a project wrapper decorator, meaning a custom decorator composing `@Controller()` or `@Get()`, are resolved and traced too.
...
Overlays real per-class construction times from a captured boot onto the modules graph. NestJS records `initTime` for every provider and controller during dependency injection. Passing `{ snapshot: true }` assembles them into a `SerializedGraph` you can dump locally, which needs `@nestjs/core` 9.3.10 or newer.

Add this to `main.ts`, for development only:
...
Nest 11.1.4 and newer can also record per-class `onModuleInit` and `onApplicationBootstrap` durations. Declare `const hookTimings: { className: string; hook: string; ms: number }[] = []` above `create()`, pass the `instrument` option below to it, then include `hookTimings` in the `Object.assign`:
...
| Proportional bars | Per class, slowest first | Color-coded by type: provider, controller, injectable, middleware |
...
- Timings are display-only. They never affect the score, diagnostics, or exit
  codes.
- An unreadable or unrecognized dump degrades to a stderr warning, and the
