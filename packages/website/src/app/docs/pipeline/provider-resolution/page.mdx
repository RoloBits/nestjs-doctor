...
- Only classes with `@Injectable()` are resolved. Controllers (which use `@Controller()`) are not included in the providers map — they're handled separately during rule execution.
