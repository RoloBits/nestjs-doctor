...
A sub-project's own score is then recalculated from the diagnostics the merge kept, over its own file count. That keeps the breakdown table consistent with the combined counts, at the cost of a sub-project scoring higher than it would alone.
