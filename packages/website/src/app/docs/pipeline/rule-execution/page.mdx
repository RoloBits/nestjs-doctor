...
Each rule call is wrapped in a try-catch. If a rule throws an exception, it's recorded as a `RuleError` and the pipeline continues. A failing rule never crashes the scan.
...
- If a rule isn't running, check that it's not disabled in config (`rules` or `categories`).
...
- The `report()` callback is the only way to emit diagnostics. If a rule doesn't call `report()`, no diagnostics are produced.
