...
- The overview lane and the axis spanned the rows' scrollbar, so a phase edge sat a few pixels right of its guide in the rows; the lanes now pad by the scrollbar's width, measured again whenever the view resizes or a dock reveals it, so the compact timeline in the modules graph's trace dock lines up too. Each handover guide is 2px wide and carries the boundary's time in the phase's color, pinned near the top while the rows scroll.
