...
- The whole timeline shares one piecewise scale. When a column is widened, the bars, hook spans, guides, axis ticks, and the minimap window move with it, so a widened `<1ms` column no longer sits over 16ms of linear axis. The axis marks each widened stretch with the same dashed weave, the crosshair chip inverts the scale so it always reads true time, and every printed label stays a real time; only the distance between labels bends, by 4.4% on the demo boot and never by more than the widened columns' minimum shares.
